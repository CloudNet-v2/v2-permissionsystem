package eu.cloudnetservice.cloudpermission.module.command;
...
import eu.cloudnetservice.cloudpermission.module.network.packet.out.PacketOutUpdateOfflinePlayer;
...
public final class CommandPermissions extends Command {
        ...
        if (args.length > 0) {
            ...
            if (args[0].equalsIgnoreCase("user")) {
                if (args.length == 2) {
                    ...
                    if (uniqueId != null) {
                        OfflinePlayer offlinePlayer = PermissionModule.getInstance().getPlayerDatabase().getPlayer(uniqueId);
    ...
    private void updatePlayer(OfflinePlayer offlinePlayer) {
        PermissionModule.getInstance().getPlayerDatabase().updatePlayer(offlinePlayer);
        if (CloudNet.getInstance().getNetworkManager().getOnlinePlayers().containsKey(offlinePlayer.getUniqueId())) {
            CloudNet.getInstance().getNetworkManager().getOnlinePlayers().get(offlinePlayer.getUniqueId())
                    .setPermissionEntity(getOld(offlinePlayer.getPermissionEntity()));
