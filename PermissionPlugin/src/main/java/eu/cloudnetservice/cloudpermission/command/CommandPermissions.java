package eu.cloudnetservice.cloudpermission.command;
...
public class CommandPermissions extends Command implements TabExecutor {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        PermissionPool permissionPool = PermissionAPI.getInstance().getPermissionPool();
        if (args.length > 0) {
                ...
                if (args.length == 4) {
                    if (args[2].equalsIgnoreCase("setDisplay")) {
                        if (permissionPool.getGroups().containsKey(args[1])) {
                            ...
                            PermissionAPI.getInstance().updatePermissionGroup(permissionGroup);
            ...
            if (args[0].equalsIgnoreCase("user")) {
                if (args.length == 2) {
                    ...
                    if (uniqueId != null) {
                        OfflinePlayer offlinePlayer = PermissionAPI.getInstance().getOfflinePlayer(uniqueId);
                        if (offlinePlayer != null && offlinePlayer.getPermissionEntity() != null) {
                            ...
                            for (GroupEntityData groupEntityData : offlinePlayer.getPermissionEntity().getGroups()) {
                                stringBuilder.append(groupEntityData.getGroup()).append('@').append(groupEntityData.getTimeout() == 0 || groupEntityData
                                                                                                                                             .getTimeout() == -1
                                                                                                    ? "LIFETIME"
                                                                                                    : simpleDateFormat.format(
                                                                                                        groupEntityData.getTimeout())).append(
    ...
    private void updatePlayer(OfflinePlayer offlinePlayer) {
        PermissionAPI.getInstance().updatePlayer(offlinePlayer);
    }
        ...
        switch (strings.length) {
            ...
            case 2: {
                switch (strings[0].toLowerCase(Locale.ENGLISH)) {
                    ...
                    case "user": {
                        return CloudAPI.getInstance().getOnlinePlayers().stream().map(de.dytanic.cloudnet.lib.player.OfflinePlayer::getName).collect(
                            Collectors.toList());
                    }
                }
            }
